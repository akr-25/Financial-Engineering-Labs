...
for i = 1: length(N)
    ...
    for j = 1 : N_ITR
        ...
        X_tilde = BS_Milstein_first_order(0.75, 0.3, 307, del_t, timepts, W); 
...
title('milstein first order for black-scholes'); 
...
function [X_hat] = BS_Milstein_first_order(mu, sigma, X_init, del_t, timepts, W) 
    n = length(timepts); 
    X_hat = timepts; 
...
function [X_hat] = BS_Milstein_second_order(mu, sigma, X_init, del_t, timepts, W, I) 
    n = length(timepts); 
    X_hat = timepts; 
