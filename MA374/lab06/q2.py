...
def plot_data(data, tickers, title):
    ...
    for ticker in tickers:
        ...
        ReturnDaily = (data[ticker] - data[ticker].shift(1))[1:]
        ReturnMean = ReturnDaily.mean()
        ReturnStd = ReturnDaily.std()
        ReturnDailyNormalized = (ReturnDaily - ReturnMean) / ReturnStd
        axarr[0].hist(ReturnDailyNormalized, bins = 60, rwidth = 0.85, density=True)
        axarr[0].set_title('Daily')
        axarr[0].plot(X, Y)

        ReturnWeekly = (data[ticker].resample('W').last() - data[ticker].resample('W').last().shift(1))[1:]
        ReturnMean = ReturnWeekly.mean()
        ReturnStd = ReturnWeekly.std()
        ReturnWeeklyNormalized = (ReturnWeekly - ReturnMean) / ReturnStd
        axarr[1].hist(ReturnWeeklyNormalized, bins = 60, rwidth = 0.85, density=True)
        axarr[1].plot(X, Y)
        axarr[1].set_title('Weekly')


        ReturnMonthly = (data[ticker].resample('M').last()[1:] - data[ticker].resample('M').last().shift(1))[1:]
        ReturnMean = ReturnMonthly.mean()
        ReturnStd = ReturnMonthly.std()
        ReturnMonthlyNormalized = (ReturnMonthly - ReturnMean) / ReturnStd
        axarr[2].hist(ReturnMonthlyNormalized, bins = 60, rwidth = 0.85, density=True)
        axarr[2].plot(X, Y)
        axarr[2].set_title('Monthly')

        plt.savefig(f'./fig/2/{ticker}_hist.png')
        plt.close()
        ...
        plt.savefig(f'./fig/2/{ticker}_box.png')
        plt.close()
        ...
        plt.savefig(f'./fig/2/{ticker}_qq.png')
        plt.close()
