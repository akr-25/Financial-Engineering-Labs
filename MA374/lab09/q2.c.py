...
def compare_volatility(file, opt_type):
  ...
  stocks_df = stocks_df.interpolate(method='pad', axis=0).ffill().bfill()
  ...
  for index, row in options_df.iterrows():
    ...
    if coin_toss <= bound:
      ...
      if int(row['Call Price']) != 0 and delta.days != 0 and not(math.isnan(row['Stock Price'])):
        ...
        opt_name = opt_type + '.NS'
        returns = reduced_df[opt_name]
        x = returns.to_list()
...
def main():
  Stocks = ["KOTAKBANK", "INFY"]
  for stock in Stocks:
    filename = stock + "optiondata.csv"
    opt_type = stock
    compare_volatility(filename, opt_type)
