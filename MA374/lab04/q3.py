...
def plotGraph(x, y, x_axis, y_axis, title, debug=False):
  plt.plot(x, y)
  plt.xlabel(x_axis)
  plt.ylabel(y_axis) 
  plt.title(title)
...
def plotGraph_both(x1, y1, x2, y2, x_axis, y_axis, title):
...
def main():
  ...
  plotGraph_both(risk, returns, risk_cml, returns_cml, "Risk (sigma)", "Returns", "Capital Market Line with Markowitz Efficient Frontier")
  plotGraph(risk_cml, returns_cml, "Risk (sigma)", "Returns", "Capital Market Line")
